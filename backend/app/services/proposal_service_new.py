...
class ProposalGenerationService:
    ...
    async def process_request(self, request: ProposalRequest) -> ProposalResponse:
        ...
        """
        self.logger.info(
            f"Processing proposal request with {len(request.sections)} sections using model: {request.model}"
        )
        ...
        self.logger.info(
            f"Completed processing proposal request with status: {overall_status}"
        )
    ...
    async def _process_section(
        ...
        self.logger.debug(f"Processing section: {section_name}")
        self.logger.debug(
            f"Section has {len(section.attached_files or [])} attached files and {len(section.attached_urls or [])} attached URLs"
        )

        # Skip generation if no user input provided
        if not section.user_input.strip():
            self.logger.warning(
                f"No user input provided for section {section_name}, skipping generation"
            )
        ...
        if (section.attached_files and len(section.attached_files) > 0) or (
            ...
            self.logger.info(
                f"Generated {len(attachment_summaries)} summaries for section {section_name}"
            )

        # Generate content using LLM service
        if attachment_summaries:
            self.logger.debug(
                f"Using LLM with {len(attachment_summaries)} attachment summaries for section {section_name}"
            )
            ...
            self.logger.debug(
                f"Using LLM without attachments for section {section_name}"
            )
        ...
        self.logger.info(
            f"Successfully generated content for section {section_name}, length: {len(generated_content)} characters"
        )
    ...
    async def _process_attachments(self, section: Section) -> List[str]:
        ...
        self.logger.debug(
            f"Processing {len(section.attached_files or [])} files and {len(section.attached_urls or [])} URLs"
        )
        ...
        for file_id in section.attached_files or []:
            ...
            try:
                ...
                if file_content:
                    self.logger.debug(
                        f"Got file content, length: {len(file_content)} characters"
                    )
